...
export default function MovieCard({ movie, size = 'md' }: MovieCardProps) {
  ...
  return (
    ...
    >
      {/* Poster */}
      <Link to={detailUrl} className={`relative ${heightClass} rounded-xl overflow-hidden bg-gray-900 block transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-red-900/30 group-hover:-translate-y-1 group-hover:scale-[1.02]`}>
        <img
          src={posterUrl}
          alt={movie.title}
