...
import { searchMovies, fetchShowboxDetail, fetchMovieStream, fetchTVShowStream, fetchAnimeStream } from '@/lib/api';
import { MASTER_API_BASE } from '@/constants';
...
// ─── API constants ────────────────────────────────────────────────────────────
const SB_BASE = `${MASTER_API_BASE}/api`;
...
// ─── Call master streaming API (replaces Supabase stream-proxy edge function) ─
async function fetchPlayData(params: {
  ...
  const { subjectId, type = 'movie', season, episode, title } = params;

  // Route to correct master API endpoint based on type
  let stream: StreamQuality | null = null;

  if (type === 'tv' && season !== undefined && episode !== undefined) {
    stream = await fetchTVShowStream(subjectId, season, episode, title);
  } else {
    stream = await fetchMovieStream(subjectId, title);
  }

  // If master API returned a stream, wrap it in PlayResult shape
  if (stream?.proxyUrl) {
    const rawStreams: StreamQuality[] = [stream];
    return { streams: rawStreams, subtitles: [], audioTracks: [] };
  }

  // Fallback: try SB_BASE direct endpoints
  const fallbackPaths =
    type === 'tv' && season !== undefined && episode !== undefined
      ? [`${SB_BASE}/stream?id=${subjectId}&type=tv&season=${season}&episode=${episode}`]
      : [`${SB_BASE}/stream?id=${subjectId}&type=movie`, `${SB_BASE}/bff/stream?subjectId=${subjectId}`];
  ...
  for (const url of fallbackPaths) {
    try {
      ...
      const candidates = Array.isArray(json.streams)
        ? json.streams
        : (json.stream && typeof json.stream === 'object' ? [json.stream] : []);
      const rawStreams: StreamQuality[] = candidates.map((s: Record<string, unknown>) => ({
      ...
      if (rawStreams.length > 0) return { streams: rawStreams, subtitles: [], audioTracks: [] };
    } catch { /* continue */ }
  }
...
export default function WatchPage() {
  ...
  useEffect(() => {
    ...
    // For embed-only sources, no stream fetch needed
    if (embedSource !== 'xcasper') { setStreamLoading(false); return; }

    if (isAnime && effectiveAnilistId) {
      // Anime: load stream via master API /api/anime
      setStreamLoading(true);
      setStreamError(false);
      fetchAnimeStream(effectiveAnilistId).then(stream => {
        if (stream?.proxyUrl) {
          setStreams([stream]);
        }
        setStreamLoading(false);
